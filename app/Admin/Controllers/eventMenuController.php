...
class eventMenuController extends Controller
{
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header('오늘의행사');
            $content->description('환경설정');

            $cnt = Preferences::where('menu', 'event')->count();

            if ($cnt > 0) {
                $config = Preferences::where('menu', 'event')->first();
    ...
    protected function form()
    {
        return Admin::form(Preferences::class, function (Form $form) {
            ...
            if ($cnt > 0) {
                ...
                $config = Preferences::where('menu', 'event')->first();
                $form->setAction('/admin/event/config/' . $config->id);

            } else {

                $form->setAction('/admin/event/config');
